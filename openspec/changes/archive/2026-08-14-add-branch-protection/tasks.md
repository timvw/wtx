...
- [x] 5.1 Merge the pull request and let the Settings App reconcile on the push to `main`.
- [x] 5.2 Confirm the rule was applied: `gh api /repos/timvw/wtx/branches/main/protection` now returns 200 rather than 404.
- [x] 5.3 Confirm the applied values: `required_status_checks.contexts` holds the five names, `required_status_checks.strict` is `false`, `enforce_admins.enabled` is `false`, and `required_pull_request_reviews` is absent from the response.
- [x] 5.4 Not needed -- the rule applied on the first reconciliation, so the App's delivery log was never consulted. If nothing was applied, read the Settings App delivery log (repo Settings → GitHub Apps → Settings) before changing anything by hand. The App reports malformed configuration to its own log, not as a check on the pull request — a silent no-op looks identical to success from the repository's side.
- [x] 5.5 Confirm on the next pull request that all five checks appear under "Required" and that the merge button is blocked until they report. Verified on the archive pull request (#11): `mergeStateStatus` read `BLOCKED` with all five checks queued, and flipped to `CLEAN` only once every one had reported success.
- [x] 5.6 Confirm a direct push to `main` is now refused -- for every actor the rule is enforced against. `enforce_admins: false` deliberately exempts admins, so the sole maintainer's own direct push is still accepted; that is the escape hatch working as designed, not a gap. Verified by the applied configuration rather than by attempting a push.
